...
class NotificationService {
    ...
    try {
      await _plugin.zonedSchedule(
        ...
        matchDateTimeComponents:
            reminder.isDaily ? DateTimeComponents.time : null, uiLocalNotificationDateInterpretation: UILocalNotificationDateInterpretation.absoluteTime,
      );
    } on MissingPluginException catch (e) {
  ...
  int _notificationId(String reminderId) => reminderId.hashCode.abs();
  ...
  DateTime _nextValidDate(DateTime date, bool isDaily) {
    ...
    if (!date.isAfter(now)) return now.add(const Duration(seconds: 5));
    return date;
  }
}
